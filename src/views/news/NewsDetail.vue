	...
	<div class="news-container">
		...
		<div class="proposal-container" v-if="tokenSupplyInfo.isVoting">
			<div class="proposal-left">
				...
				<div class="proposal-left-link" @click="$refs['nftStakeDialog'].showDialog()">View the latest NFT
					infomation >></div>
			</div>
			...
			<div class="form-top">
				<div class="form-left">
					...
					<div style="display: flex;flex-direction: column;align-items: center;" v-else>
						...
						<el-button @click="handleUnlock()" class="common-btn2"
							style="border-radius: 25px;margin-top: 29px;">Unlock</el-button>
					</div>
					<div class="form-tag" v-if="metadata.keyword && metadata.keyword.length">
						<div class="form-tag-label text-color">Tags:</div>
						<div class="form-tag-item text-color" v-for="(item,index) in metadata.keyword" :key="index">
							{{ item }}</div>
					</div>
					<div class="dao-container">
						...
						<div class="dao-set">
							<div class="dao-set-item" style="width: 158px;"
								@click="$router.push( { path: '/update', query: {'tokenId': tokenId, 'step': 1}})">
								<div class="dao-set-item-label">Update NFT Attributes</div>
								<i class="el-icon-arrow-right"></i>
							</div>
							<div class="dao-set-item" style="width: 158px;"
								@click="$router.push( { path: '/update', query: {'tokenId': tokenId, 'step': 2}})">
						...
						<div class="dao-set-tip">
							...
							<span class="text-color" style="cursor: pointer;"
						...
						<div class="form-attr-market">
							...
							<div class="form-attr-mbd">
								...
								<div class="mbd-transform">
									≈${{ (metadata.initialPrice * $store.state.chain.mbdPrice) | decimalPlace8 }}</div>
							</div>
							<el-button @click="handleMint()" :disabled="metadata.availableSupply <= 0"
								class="common-btn2 form-attr-mint">Mint</el-button>
					...
					<div class="form-attr-container">
						...
						<div class="form-second-market">
							<div class="second-market-column" style="width: 130px;">
								...
								<div class="second-market-td" style="padding-left: 14px;"
									v-for="(item,index) in nftOrderList" :key="index">{{ item.owner | omitAddress }}
								</div>
							</div>
							<div class="second-market-column" style="text-align: right;width: 91px;">
								<div class="second-market-header">Price(MBD)</div>
								<div class="second-market-td" v-for="(item,index) in nftOrderList" :key="index">
									{{ item.price }}</div>
							</div>
							<div class="second-market-column" style="text-align: right;width: 104px;">
								...
								<div class="second-market-td" style="padding-right: 12px;"
									v-for="(item,index) in nftOrderList" :key="index">{{ item.tokenValue }}</div>
							</div>
							<div class="second-market-column" style="width: 60px;">
								<div class="second-market-header"></div>
								<div class="second-market-td" style="font-size: 12px;"
									v-for="(item,index) in nftOrderList" :key="index">
									<div class="second-btn">
										<span v-if="item.owner != $store.state.user.account"
											style="color: #00F9E5;">Buy</span>
						...
						<div class="form-dao">
							...
							<div class="dividend-pool">
								<div class="dividend-pool-item">
									...
									<div class="dividend-pool-value text-color">{{ settlePoolBalance | toLocalString }}
										MBD</div>
								</div>
								<div class="dividend-pool-item">
								...
								<div class="dividend-pool-item">
									...
									<div class="dividend-pool-value text-color"
										v-if="userStakeInfo && userStakeInfo[0]">{{ userStakeInfo[0] }}
										({{ stakePercent }})</div>
		...
		<RetrieveDialog :tokenId="tokenId" :blockHeight="currentHeight" :userStakeInfo="userStakeInfo"
			ref="retrieveDialog" />
