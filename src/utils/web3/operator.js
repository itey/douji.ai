...
function getOperatorContract() {
  ...
  if (contract) {
    ...
    contract = new web3.eth.Contract(operator.abi, process.env.VUE_APP_OPERATOR)
    return contract
  }
}
...
export function checkInContract() {
  ...
  return new Promise((resolve, reject) => {
    operatorContract.methods.checkIn()
...
export function getBoxContract(tokenId) {
  ...
  const fromAddress = store.state.chain.account
  return new Promise((resolve, reject) => {
    operatorContract.methods.getBox(tokenId)
      .send({ from: fromAddress })
      .on('transactionHash', (hash) => {
        console.log('transactionHash:', hash)
      })
...
export function openBoxContract() {
  ...
  const fromAddress = store.state.chain.account
  return new Promise((resolve, reject) => {
    operatorContract.methods.openBox()
      .send({ from: fromAddress })
      .on('transactionHash', (hash) => {
        console.log('transactionHash:', hash)
      })
